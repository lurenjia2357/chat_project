    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            RoundButton {
                ...
                onClicked: {
                    if (email_edit.text === "" || password_edit.text === "" ||
                        confirm_edit.text === "" || verify_edit.text === "") {
                    ...
                    sure_button.text = "重置中..."

                    var body = "email=" + encodeURIComponent(email_edit.text)
                               + "&password=" + encodeURIComponent(password_edit.text)
                               + "&code=" + encodeURIComponent(verify_edit.text)

                        var xhr = new XMLHttpRequest()
                        xhr.open("POST", "http://127.0.0.1:10086/reset_password")
                    ...
                    xhr.onreadystatechange = function() {
                        ...
                        if (xhr.status === 200) {
                            error_tip.text = "密码重置成功！五秒后返回登录界面···"
                            error_tip.color = "green"
                            countdown_timer.triggered.connect(function() {
                                _reset_page.sigSwitchLogin()
                            })

                            countdown_timer.start()
                        } else if (xhr.status === 409) {
                            error_tip.text = "该邮箱未注册或请求冲突"
                            error_tip.color = "red"
            ...
            RoundButton {
                ...
                text: "取消"
                radius: 5
                onClicked: _reset_page.sigSwitchLogin()
            }
        }
